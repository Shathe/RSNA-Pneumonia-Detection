...
def FCMnasNet(n_classes=1000, input_shape=(224, 224, 3), alpha=1):
	...
	x1 = MBConv_idskip(x, filters=24, kernel_size=3,  strides=1, filters_multiplier=3, alpha=alpha)
	# MBConv3 5x5
	x = MBConv_idskip(x1, filters=40, kernel_size=5,  strides=2, filters_multiplier=3, alpha=alpha)
	x = MBConv_idskip(x, filters=40, kernel_size=5,  strides=1, filters_multiplier=3, alpha=alpha)
	x2 = MBConv_idskip(x, filters=40, kernel_size=5,  strides=1, filters_multiplier=3, alpha=alpha)
	# MBConv6 5x5
	x = MBConv_idskip(x2, filters=80, kernel_size=5,  strides=2, filters_multiplier=6, alpha=alpha)
...
# Convolution with batch normalization
def transpose_conv_bn(x, filters, kernel_size,  strides=1, alpha=1, activation=True):
	"""Convolution Block
	This function defines a 2D convolution operation with BN and relu6.
	# Arguments
		x: Tensor, input tensor of conv layer.
	...
	"""
	filters = _make_divisible(filters * alpha)
	x = layers.Conv2DTranspose(filters=filters, kernel_size=kernel_size, strides=strides, padding='same',
									use_bias=False, kernel_regularizer=regularizers.l2(l=0.0003))(x)
	x = layers.BatchNormalization(epsilon=1e-3, momentum=0.999)(x)  
	if activation:
		x = layers.ReLU(max_value=6)(x)
...
if __name__ == "__main__":
	...
	model = FCMnasNet(n_classes=1, input_shape=(320, 320, 1))
